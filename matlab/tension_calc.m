...
teta_fc=0;     % forearm clamping angle in radians.
teta_ac=0;     % arm clamping angle in radians
...
%             Dcf(j,1)=Dcf_max-(j-1)*Dcf_step;
            ...
            x=sqrt(Dca^2+Dcf^2-2*Dca*Dcf*cos(phi));

            %angles calculations
            teta_fc=asin((Dca*sin(phi))/x);
            teta_ac=asin((Dcf*sin(phi))/x);

            %Clamping force calculation
            Tfc=Wf*Dfcm*g*sin(phi);
            Td=Wd*Dd*g*sin(phi);
            Fcf=(Tfc+Td)/(Dca*sin(teta_ac));
%        end
%    end
