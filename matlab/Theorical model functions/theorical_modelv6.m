...
for i=2:size(speed_hyst,1)      %construction of the hysteresis based the speed
    if(speed_hyst(i,1)-Upper_lim_speed>0)
        h1(i,1)=min(1,h1(i-1,1)+stepup_sgm);
        h2(i,1)=max(0,h2(i-1,1)-stepdown_sgm);
    else
        if(speed_hyst(i,1)-Lower_lim_speed<0)
...
legend('Loadcell measure','Model signal','Error')
subplot(2,1,2)
plot(axisx,Tc./Dcf.*sin(Beta),axisx,Fc_least_squares./Dcf.*sin(Beta),axisx,Error_newtons,'r',axisx,max(Error_newtons),'k',axisx,min(Error_newtons),'k')
